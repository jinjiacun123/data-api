...
def init(file_dir):
    ...
    while True:
        ...
        file_name = "%s_%s.data" % (code_type,code)
...
def add_code(file_dir, code_type, code, price, time):
    print 'add_code code_type:%s,code:%s' % (code_type,code)
    file_name = "%s%s_%s.data" % (file_dir, code_type, code)
    s = struct.pack("ii", price, time)
