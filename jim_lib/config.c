...
#define PARSE_STATUS_GET_KEY_OR_SECTION1
#define PARSE_STATUS_GET_SECTION2
#define PARSE_STATUS_GET_VALUE3
#define PARSE_STATUS_COMMENT_LINE4

CONFIG_BTREE_NODE* config_btree_find_node(CONFIG_BTREE* config,const char* key);

/**
 * 鍐呴儴浣跨敤鍑芥暟,瀹炵幇鍐呭瓨娈祎rim,杩斿洖绗竴涓潪绌哄瓧绗︽寚閽堝強瀛楃涓瞭rim鍚庣殑闀垮害.
 */
...
/**
 * ini鏂囦欢瑙ｆ瀽鍑芥暟璺宠浆琛�,姝ゆ柟寮忓湪澶у瀷瑙ｆ瀽鐨勫疄鐜颁腑闈炲父楂樻晥.
 */
...
INI_CONFIG* ini_config_create_from_string(unsigned char* str,int slen,int isGBK){
  ...
  while(parser.pos<slen){
    ...
    if(!r){ //瑙ｆ瀽閿欒,鏈唬鐮佷笉鍋氫换浣曟彁绀�,鐩存帴杩斿洖锛嫉锛棘,浣嗗彲浠ヤ粠parser閲屽湪杩欓噷浠巔arser閲屽彇寰楀綋鍓嶈В鏋愮殑鏂囦欢浣嶇疆鍜屽綋鍓嶇姸鎬�.
      config_btree_destroy(config);
      return NULL;
    }
  }
