...
t_base_c_request_head * head;
{
	...
	data.m_lBeginPosition =0;          //请求开始位置
        data.m_nPeriodNum =1;
	data.m_nDay = 100*data.m_nPeriodNum;//请求个数(10个)
	data.m_nSize2 =0;
     ...
     buff_t * my_buff;
{
  ...
  char code[7] = {0};
	...
	if(pHisData222->m_nType == TYPE_HISTORY){
	  ...
	  memcpy(code,g->m_dhHead.m_nPrivateKey.m_pCode.m_cCode2,6);
...
insert_into(code_type, code ,date, open, max, min, close, money, total)
     ...
     long total;
{
  char * template_sql = "insert into hr_history_day(code_type, \
                                               code, \
                                               m_date, \
                                               open_price, \
                                               close, \
                                               ...
                                               money, \
                                               total) \
  values(%ld,%s,%ld,%ld,%ld,%ld,%ld,%ld,%ld);";
  char * sql[1024];
  memset(sql, 0, 1024);
